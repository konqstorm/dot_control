...
3. Посчитать метрики для обученной модели:
```
python scripts/evaluate.py
```

4. Запустить live‑визуализацию обученной модели (откроется окно):
```
python scripts/run.py
```

Параметры окружения, обучения, валидации и визуализации регулируются через `config/config.yaml`. Готовые результаты обучения и метрики сохраняются в artifacts, там же можно найти чекпоинт лучшей обученной модели.
...
![Rollout Visualization](artifacts/rollout.gif)
